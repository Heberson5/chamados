...
-- Updating audit_logs.user_id pointing to profiles
...
CREATE POLICY "User view own chamados" ON public.chamados FOR SELECT USING (usuario_id = my_id());
DROP POLICY IF EXISTS "User create own chamados" ON public.chamados;
CREATE POLICY "User create own chamados" ON public.chamados FOR INSERT WITH CHECK (usuario_id = my_id());
