...
DO $$ BEGIN
  EXECUTE (SELECT 'ALTER POLICY ' || quote_ident(policyname) || ' ON public.profiles USING (auth.uid() = user_id)'
           FROM pg_policies WHERE tablename = 'profiles' AND schemaname = 'public' AND (qual LIKE '%(auth.uid() = id)%' OR qual LIKE '%(id = auth.uid())%'));
EXCEPTION WHEN OTHERS THEN END $$;

-- Processing table: audit_logs
ALTER TABLE public.audit_logs DROP CONSTRAINT IF EXISTS audit_logs_pkey CASCADE;
ALTER TABLE public.audit_logs RENAME COLUMN id TO old_id;
ALTER TABLE public.audit_logs RENAME COLUMN id_numerico TO id;
ALTER TABLE public.audit_logs ADD PRIMARY KEY (id);

-- Processing table: baixas
...
-- Updating FK baixa_id in itens_baixa
...
-- Updating FK categoria_id in itens_inventario
...
-- Updating FK chamado_pai_id in chamados
...
-- Updating FK chamado_id in ordens_de_servico
...
-- Updating FK chamado_id in transferencias_chamado
...
-- Updating FK chamado_id in comentarios_chamado
...
-- Processing table: chamados_prioridades
...
-- Updating FK prioridade_id in chamados
...
-- Processing table: comentarios_chamado
...
-- Processing table: departamentos
...
-- Updating FK department_id in chamados
...
-- Updating FK department_id in profiles
...
-- Processing table: estoque_setor
...
-- Updating FK fornecedor_id in solicitacoes_compra
...
-- Processing table: help_menu_manuals
...
-- Processing table: itens_inventario
...
-- Updating FK item_id in movimentacoes_estoque
...
-- Updating FK item_inventario_id in estoque_setor
...
-- Updating FK item_inventario_id in itens_solicitacao_compra
...
-- Updating FK item_inventario_id in itens_baixa
...
-- Processing table: itens_solicitacao_compra
...
-- Processing table: movimentacoes_estoque
...
-- Processing table: ordens_de_servico
...
-- Updating FK organization_id in profiles
...
-- Updating FK organization_id in user_roles
...
-- Updating FK organization_id in departamentos
...
-- Updating FK organization_id in chamados_prioridades
...
-- Updating FK usuario_id in expedientes
...
-- Updating FK tecnico_id in chamados
...
-- Updating FK prioridade_alterada_por in chamados
...
-- Updating FK vinculado_por in chamados
...
-- Updating FK tecnico_anterior_id in transferencias_chamado
...
-- Updating FK tecnico_novo_id in transferencias_chamado
...
-- Updating FK transferido_por in transferencias_chamado
...
-- Updating FK autor_id in comentarios_chamado
...
-- Updating FK solicitante_id in reembolsos
...
-- Updating FK aprovador_id in reembolsos
...
-- Updating FK pagador_id in reembolsos
...
-- Updating FK criado_por in itens_inventario
...
-- Updating FK realizado_por in movimentacoes_estoque
...
-- Updating FK solicitado_por in solicitacoes_compra
...
-- Updating FK aprovado_por in solicitacoes_compra
...
-- Updating FK rejeitado_por in solicitacoes_compra
...
-- Updating FK executado_por in solicitacoes_compra
...
-- Updating FK solicitado_por in baixas
...
-- Updating FK aprovado_tecnico_por in baixas
...
-- Updating FK aprovado_gestor_por in baixas
...
-- Updating FK rejeitado_por in baixas
...
-- Updating FK executado_por in baixas
...
-- Updating FK usuario_id in chamados
...
-- Updating FK user_id in audit_logs
ALTER TABLE public.audit_logs ADD COLUMN IF NOT EXISTS user_id_new BIGINT;
UPDATE public.audit_logs t SET user_id_new = r.id FROM public.profiles r WHERE t.user_id = r.old_uuid_id;
ALTER TABLE public.audit_logs DROP COLUMN IF EXISTS user_id;
ALTER TABLE public.audit_logs RENAME COLUMN user_id_new TO user_id;
ALTER TABLE public.audit_logs ADD CONSTRAINT audit_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id);

-- Processing table: reembolsos
...
-- Processing table: role_definitions
...
-- Updating FK servico_id in ordens_de_servico
...
-- Processing table: solicitacoes_compra
...
-- Updating FK solicitacao_compra_id in itens_solicitacao_compra
...
-- Processing table: system_manuals
...
ALTER TABLE public.audit_logs DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.audit_logs DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.baixas DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.baixas DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.categorias DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.categorias DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.chamados DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.chamados DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.chamados_prioridades DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.chamados_prioridades DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.comentarios_chamado DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.comentarios_chamado DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.departamentos DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.departamentos DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.estoque_setor DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.estoque_setor DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.expedientes DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.expedientes DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.fornecedores DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.fornecedores DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.help_menu_manuals DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.help_menu_manuals DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.itens_baixa DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.itens_baixa DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.itens_inventario DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.itens_inventario DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.itens_solicitacao_compra DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.itens_solicitacao_compra DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.movimentacoes_estoque DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.movimentacoes_estoque DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.notificacoes DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.notificacoes DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.ordens_de_servico DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.ordens_de_servico DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.organizations DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.organizations DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.profiles DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.profiles DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.reembolsos DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.reembolsos DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.role_definitions DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.role_definitions DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.servicos DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.servicos DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.solicitacoes_compra DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.solicitacoes_compra DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.system_manuals DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.system_manuals DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.system_settings DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.system_settings DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.transferencias_chamado DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.transferencias_chamado DROP COLUMN IF EXISTS old_id;
ALTER TABLE public.user_roles DROP COLUMN IF EXISTS old_uuid_id;
ALTER TABLE public.user_roles DROP COLUMN IF EXISTS old_id;